...
// never touches the development uml_architect database. It is unique per test
// binary (process) so `go test ./...` can run packages in parallel: two
// packages sharing one fixed name raced and one skipped with a duplicate-key
// error on CREATE.
var testDBName = fmt.Sprintf("uml_architect_gobe02_test_%d", os.Getpid())
