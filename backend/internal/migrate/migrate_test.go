...
// TestV3SeedsCredentialUserIdempotently pins the V3 contract without naming
// any credential material: the seed INSERT must skip on conflict and the
// hash alignment UPDATE must rewrite the password_hash column. The actual
// email and hash live only in the V3 SQL file, never in Go code.
func TestV3SeedsCredentialUserIdempotently(t *testing.T) {
	migs := migrate.Ordered()
	if len(migs) < 3 {
		t.Fatalf("expected V3 migration, got %d migrations", len(migs))
	}
	v3 := migs[2].SQL
	for _, want := range []string{
		...
		if !contains(v3, want) {
			t.Errorf("V3 must contain idempotent seed semantics %q", want)
		}
	}
}
