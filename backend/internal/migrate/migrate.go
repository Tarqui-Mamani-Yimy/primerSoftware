...
// version order (V1__, V2__, V3__, ...).
//
// V1 is the sole schema authority for the Go-owned English tables (users,
// projects, diagrams, diagram_versions, refresh_tokens, project_memberships);
// V2 seeds the initial diagram with conditional semantics; V3 seeds the
// credential user and aligns the development password hashes. The schema only
// ever changes through these versioned files — never through automatic
// schema updates. Applied versions are journaled in go_schema_migrations so
...
// applying V1__ then V2__ then V3__ and so on.
