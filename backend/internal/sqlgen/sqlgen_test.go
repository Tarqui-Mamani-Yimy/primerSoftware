...
func TestRenderSQLStructure(t *testing.T) {
	...
	wants := []string{
		...
		"CREATE TABLE IF NOT EXISTS rel_course__tag (",
		...
		"ALTER TABLE rel_course__tag ADD CONSTRAINT fk_rel_course__tag__course_id FOREIGN KEY (course_id) REFERENCES course (id);",
		"ALTER TABLE rel_course__tag ADD CONSTRAINT fk_rel_course__tag__tag_id FOREIGN KEY (tag_id) REFERENCES tag (id);",
