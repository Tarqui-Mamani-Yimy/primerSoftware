...
export const authApi={ login:(email:string,password:string)=>request<AuthenticatedUser>('/auth/login',{method:'POST',body:JSON.stringify({email,password})}), setToken:(token?:string)=>{accessToken=token;} };
export const projectApi={ list:()=>request<AssignedProject[]>('/projects'), create:(input:CreateProjectInput)=>request<CreatedProject>('/projects',{method:'POST',body:JSON.stringify(input)}), join:(accessCode:string)=>request<AssignedProject>('/projects/join',{method:'POST',body:JSON.stringify({accessCode})}) };
