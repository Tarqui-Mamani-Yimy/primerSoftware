...
export function LoginScreen({ onContinue }: LoginScreenProps) {
  ...
  const [email, setEmail] = useState('architect@local.dev');
  ...
  return (
    <main className="min-h-screen bg-[#0a0e16] text-[#dfe2ee] px-5 py-8 sm:px-8 lg:px-12">
          ...
          <div className="relative flex h-full flex-col">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#98a6b8]">Local-first design workspace</p>
              </div>
            </div>

            <div className="my-auto max-w-lg py-14 lg:py-0">
              <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#2d6153] bg-[#12362c] px-3 py-1 text-xs font-semibold text-[#70e6b6]"><span className="h-1.5 w-1.5 rounded-full bg-[#70e6b6]" />Workspace ready</p>
              <h1 className="font-heading text-4xl font-bold leading-tight tracking-tight text-white sm:text-5xl">Design systems together, without losing the model.</h1>
              <p className="mt-5 max-w-md text-sm leading-7 text-[#aab6c5]">Move from an idea to a maintainable UML model with clear versions, focused collaboration, and local AI tooling.</p>
              <div className="mt-9 grid gap-4 sm:grid-cols-3">
                {[
                  { icon: <Mic className="h-4 w-4" />, label: 'Voice-ready', detail: 'Local pipeline' },
                  { icon: <Layers3 className="h-4 w-4" />, label: 'Structured UML', detail: 'Canonical model' },
                  { icon: <Sparkles className="h-4 w-4" />, label: 'Version-aware', detail: 'Safe iteration' }
                ].map(item => (
            ...
            <p className="text-xs text-[#6f7e91]">Your projects stay visible; authentication will be connected when the user domain is defined.</p>
          </div>
        </section>

        <section className="flex items-center p-7 sm:p-10 lg:p-12">
          <div className="mx-auto w-full max-w-sm">
            <p className="text-sm font-semibold text-[#62dcb1]">Welcome back</p>
            <h2 className="font-heading mt-2 text-3xl font-bold tracking-tight text-white">Enter your workspace</h2>
            <p className="mt-3 text-sm leading-6 text-[#98a6b8]">Use the local demo entry to preview your assigned projects.</p>

            <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
              <div>
                <label className="mb-2 block text-sm font-medium text-[#dfe7f1]" htmlFor={emailId}>Email</label>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-[#dfe7f1]" htmlFor={passwordId}>Password</label>
                <div className="relative">
                  <LockKeyhole className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8393a6]" aria-hidden="true" />
                  <input id={passwordId} value={password} onChange={event => setPassword(event.target.value)} type="password" autoComplete="current-password" placeholder="Your password" className="w-full rounded-lg border border-[#364352] bg-[#0d141e] py-3 pl-10 pr-3 text-sm text-white outline-none transition placeholder:text-[#667588] focus:border-[#5ce0b2] focus:ring-2 focus:ring-[#5ce0b2]/20" />
                </div>
              </div>
              <button disabled={submitting} type="submit" className="group flex w-full items-center justify-center gap-2 rounded-lg bg-[#21c78d] px-4 py-3 text-sm font-bold text-[#062d20] transition hover:bg-[#52dfae] focus:outline-none focus:ring-2 focus:ring-[#8cf3cf] focus:ring-offset-2 focus:ring-offset-[#101722]">
                {submitting ? 'Signing in…' : 'Sign in'} <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" aria-hidden="true" />
              </button>
            </form>
